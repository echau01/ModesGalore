...
public class GiveDirtCommand implements CommandExecutor {
	...
	public GiveDirtCommand(Main plugin) {
		this.plugin = plugin;
	}
					...
					if (args.length == 0) {
						...
						if (args.length == 1) {
							if (receiver != null) { //Check if the player (receiver) actually exists.
								...
								Inventory inv = Bukkit.createInventory(null, 9, ChatColor.DARK_AQUA + "Take the dirt!");
							...
							if (receiver != null) {	//Check if the player (receiver) actually exists.
								try {	//The command sender could pass in something that isn't a number.
									int dirtAmount = (int) Math.round(Double.parseDouble(args[1]));	//Turn second parameter into double, round it to the nearest number, and cast as integer
									if (0 < dirtAmount && dirtAmount <= 576) {	//dirtAmount is between 0 and 576.
										ItemStack dirt = new ItemStack(Material.DIRT, dirtAmount);	//Turns the second argument into a double, rounds it to the nearest number, and casts it as an integer.
										Inventory inv = Bukkit.createInventory(null, 9, ChatColor.DARK_AQUA + "Take the dirt!");
										...
										ItemStack dirt = new ItemStack(Material.DIRT, 576);
										Inventory inv = Bukkit.createInventory(null, 9, ChatColor.DARK_AQUA + "Take the dirt!");
										inv.addItem(dirt);	//Add 576 dirt to the newly created inventory
										receiver.openInventory(inv);	//Opens the inventory of the receiver


										int remainder = dirtAmount - 576;	//Calculates the remaining amount of dirt
										...
										int remainderStacks = (int) remainder / 64;	//Calculates number of full stacks that the remaining dirt takes up.
										int remainderStacksRemainder = remainder % 64;	//Calculates the amount of dirt in the final non-full stack. Example: 131 dirt = 2 full stacks + 1 non-full stack with 3 dirt.
										...
										if (dropDirt) {
											for(int i = 0; i <= remainderStacks; i++) {
												if (i == remainderStacks) {
													Item droppedDirt = receiver.getWorld().dropItem(receiver.getLocation(), new ItemStack(Material.DIRT, remainderStacksRemainder));
													droppedDirt.setPickupDelay(20);
												} else {
													Item droppedDirtStacks = receiver.getWorld().dropItem(receiver.getLocation(), new ItemStack(Material.DIRT, 64));
